...
# *
# * Author: Yanbo Zhang   Mail: zhangyanbo@renaissancerobot.com
# * Date: 2017.10.10
# */
...
env_name = "GazeboCircuit2TurtlebotLidar-v0"

# A3C(Asynchronous Advantage Actor Critic) agent(global) 
class A3CAgent:
    def __init__(self, inputs, outputs, discountFactor, learningRate, learnStart):
        ...
        self.discount_factor = discountFactor   # 0.99
        ...
        self.summary_writer = tf.summary.FileWriter('summary/turtlebot', self.sess.graph)

    def train(self):
        # self.load_model("./save_model/turtlebot")
        agents = [Agent(self.output_size, self.input_size, [self.actor, self.critic], 
        ...
        while True:
            time.sleep(60*10)
            self.save_model("./save_model/turtlebot")
    ...
    def actor_optimizer(self):
        action = K.placeholder(shape=[None, self.output_size])
        ...
        optimizer = optimizers.RMSprop(lr=self.actor_lr, rho=0.99, epsilon=0.01)
    ...
    def critic_optimizer(self):
        ...
        optimizer = optimizers.RMSprop(lr=self.critic_lr, rho=0.99, epsilon=0.01)
...
class Agent(threading.Thread):
    def __init__(self, output_size, input_size, memorySize, model, sess, optimizer, discount_factor, summary_ops):
    ...
    def run(self):
        ...
        while episode < EPISODES:
            ...
            while not done:
                ...
                action, policy = self.get_action(self.states[-1])
                ...
                self.avg_p_max += np.amax(self.actor.predict(state.reshape(1,len(state))))
    ...
    def discount_rewards(self, rewards, done):
        ...
        running_add = 0
        if not done:
            running_add = self.critic.predict(state.reshape(1,len(state)))[0]
    ...
    def get_action(self):
        ...
        policy = self.local_actor.predict(self.states[-1])[0]
        action_index = np.random.choice(self.output_size, 1, p=policy)[0]
...
if __name__ == "__main__":
    global_agent = A3CAgent(inputs=20, outputs=21, discountFactor=0.99, learningRate=0.00025, learnStart=64)
    global_agent.train()
